...
$stmt = $db->query("
    SELECT DISTINCT user_principal_name FROM (
        SELECT user_principal_name FROM login_logs WHERE login_time >= NOW() - INTERVAL 7 DAY AND status = 'Success'
        UNION
        SELECT user_principal_name FROM login_logs WHERE login_time >= NOW() - INTERVAL 7 DAY AND (is_impossible_travel = 1 OR is_region_change = 1)
        UNION
        SELECT user_principal_name FROM auth_device_changes WHERE change_time >= NOW() - INTERVAL 7 DAY
    ) AS users
");
